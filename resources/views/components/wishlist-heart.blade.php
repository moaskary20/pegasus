...
<div class="absolute top-2 left-2 z-10" {{ $attributes }}>
    @if($inWishlist)
        <form method="POST" action="{{ route('site.wishlist.courses.remove', $course) }}" class="inline-block" onclick="event.stopPropagation();">
            @csrf
    ...
    @else
        <form method="POST" action="{{ route('site.wishlist.courses.add', $course) }}" class="inline-block" onclick="event.stopPropagation();">
            @csrf
