@props(['href' => '#', 'label' => '', 'icon' => 'link', 'badge' => null])
@php
    $icons = [
        'courses' => '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253"/>',
        ...
        'wishlist' => '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z"/>',
        ...
        'my-courses' => '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2"/>',
    ...
    $svg = $icons[$icon] ?? $icons['link'];
@endphp
<a
    href="{{ $href }}"
    {{ $attributes->merge(['class' => 'reveal-item flex flex-col items-center justify-center p-5 rounded-2xl bg-white border border-slate-200 hover:shadow-lg hover:scale-105 hover:border-[#3d195c]/30 transition-all duration-300 group']) }}
    data-reveal
>
    <div class="relative">
        <div class="w-12 h-12 rounded-xl bg-[#3d195c]/10 flex items-center justify-center group-hover:bg-[#3d195c]/20 transition">
            <svg class="w-6 h-6 text-[#3d195c]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                {!! $svg !!}
            </svg>
        </div>
        @if($badge !== null && (int) $badge > 0)
            <span class="absolute -top-1 -left-1 min-w-[20px] h-5 px-1.5 rounded-full bg-rose-500 text-white text-xs font-bold flex items-center justify-center">
                {{ $badge > 99 ? '99+' : $badge }}
            </span>
        @endif
    </div>
    <span class="mt-2 text-sm font-bold text-slate-900">{{ $label }}</span>
</a>
