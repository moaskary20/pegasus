                        ...
                        <div class="p-5">
                            ...
                            <div class="mt-5 grid gap-2">
                                ...
                                @else
                                    ...
                                    @else
                                        @if($course->offersSubscriptionPlanChoice())
                                        {{-- اختيار خطة: شهري أو يومي مُعرّفان --}}
                                        ...
                                        @else
                                        {{-- شراء لمرة واحدة فقط: سعر العرض أو الافتراضي — بدون اختيار خطة --}}
                                        <form action="{{ route('site.course.subscribe.add_to_cart', $course) }}" method="POST" class="w-full">
                                            @csrf
                                            <input type="hidden" name="subscription_type" value="once">
                                            <button type="submit" class="w-full inline-flex items-center justify-center gap-2 px-5 py-3.5 rounded-2xl bg-[#3d195c] text-white font-extrabold hover:bg-[#3d195c]/95 transition text-base">
                                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 11-4 0 2 2 0 014 0z"/></svg>
                                                إضافة إلى السلة
                                            </button>
                                        </form>
